...
from motulator.control.common import PWM, Ctrl, ComplexPICtrl, SpeedCtrl
...
class VectorCtrl(Ctrl):
    ...
    def __call__(self, mdl):
        ...
        if self.sensorless:
            w_s = self.observer(self.T_s, u_s, i_s)
        else:
...
class CurrentReferencePars:
    ...
    def __post_init__(self, par, u_s_nom, w_s_nom):
        ...
        if self.k_fw is None:
            # TODO: Improve
            self.k_fw = par.R_R*self.psi_R_nom/(par.L_sgm*self.u_s_nom)**2
...
class CurrentReference:
    ...
    the flux-producing current component::
        ...
        i_s_ref.real = (k_fw/s)*(u_s_max**2 - abs(u_s_ref)**2)
    ...
    def update(self, T_s, u_s_ref, u_dc):
        ...
        self.i_sd_ref += T_s*self.k_fw*(u_s_max**2 - np.abs(u_s_ref)**2)
...
class Observer:
    ...
    is similar to [Ver1988]_, [Hin2010].
    
    . This implementation operates in estimated 
    rotor flux coordinates. The default gain allows smooth transition from the 
    current model at zero speed to the (damped) voltage model at higher speeds.
    ...
    k : callable, optional
        ...
        ``lambda w_m: 1 + .2*abs(w_m)/(R_R/L_M - 1j*w_m)``.
    ...
    Notes
    -----
    The observer pole could be placed arbitrarily by means of the gain `k`. The
    current model is obtained by setting ``k = lambda w_m: 1``, resulting in the 
    pole at ``s = -R_R/L_M - 1j*(w_s - w_m)`` in synchronous coordinates. The 
    pure voltage model corresponds to ``k = lambda w_m: 0``, resulting in the 
    marginally stable pole at ``s = -1j*w_s``. The default gain places the pole 
    at ``s = -R_R/L_M - .2*abs(w_m) - 1j*(w_s - w_m)``.

    References
    ----------
    .. [Ver1988] Verghese, Sanders, “Observers for flux estimation in induction
       machines,” IEEE Trans. Ind. Electron., 1988,
       https://doi.org/10.1109/41.3067

    .. [Har2001] Harnefors, “Design and analysis of general rotor-flux-oriented
       vector control systems,” IEEE Trans. Ind. Electron., 2001, 
       https://doi.org/10.1109/41.915417
    ...
    def __init__(self, par, k=None, alpha_o=2*np.pi*40, sensorless=True):
        ...
        # Other parameters
        self.alpha_o = alpha_o  # Speed-estimation bandwidth (rad/s)
        self.sensorless = sensorless
