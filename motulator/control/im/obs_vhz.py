...
class ObserverBasedVHzCtrl(Ctrl):
    ...
    def __init__(self, par, ctrl_par, T_s=250e-6):
        ...
        # Instantiate classes
        self.observer = SensorlessObserverExtCoord(par, alpha_o=2*np.pi*40)
...
# %%
class SensorlessObserverExtCoord:
    """
    ...
    for an induction machine, corresponding to [Hin2010]_. The observer gain 
    decouples the electrical and mechanical dynamics and allows placing the 
    poles of the linearized estimation error dynamics. This implementation 
    operates in external coordinates (typically synchronous coordinates defined 
    by reference signals of a control system).
