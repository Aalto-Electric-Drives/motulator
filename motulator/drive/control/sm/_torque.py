...
class TorqueCharacteristics:
    ...
    def mtpa(self, abs_i_s):
        ...
        if par.psi_f == 0:
            ...
            if par.L_q > par.L_d:
                beta = np.arccos(0.25 * (a - np.sqrt(a**2 + 8)))
            else:
                beta = np.arccos(0.25 * (a + np.sqrt(a**2 + 8)))
    ...
    def mtpv(self, abs_psi_s):
        ...
        par = self.par
        # Replace zeros with epsilon
        abs_psi_s = (abs_psi_s > 0) * abs_psi_s + (abs_psi_s <= 0) * float_info.epsilon
        ...
        if par.psi_f == 0:
            ...
            if par.L_q > par.L_d:
                delta = np.arccos(0.25 * (a - np.sqrt(a**2 + 8)))
            else:
                delta = np.arccos(0.25 * (a + np.sqrt(a**2 + 8)))
    ...
    def mtpv_current(self, abs_i_s):
        ...
        if par.psi_f == 0:
            ...
            if par.L_d == par.L_q:
                ...
                if par.L_q > par.L_d:
                    i_sd = 0.5 * (-b - np.sqrt(b**2 - 4 * a * c)) / a
                else:
                    i_sd = 0.5 * (-b + np.sqrt(b**2 - 4 * a * c)) / a
                i_sq = np.sqrt(abs_i_s**2 - i_sd**2)
                i_s = i_sd + 1j * i_sq
    ...
    def mtpv_locus(self, max_psi_s=None, max_i_s=None, N=20):
        ...
        return SimpleNamespace(
            psi_s=psi_s, i_s=i_s, tau_M=tau_M, tau_M_vs_abs_psi_s=tau_M_vs_abs_psi_s
        )
    ...
    def current_limit(self, max_i_s, gamma1=np.pi, gamma2=0, N=20):
        ...
        tau_M_vs_abs_psi_s = interp1d(
            np.abs(psi_s), tau_M, bounds_error=False, fill_value=(tau_M[0], tau_M[-1])
        )
    ...
    def plot_flux_loci(self, max_i_s, base, N=20):
        ...
        _, ax = plt.subplots()
        ax.plot(mtpa.psi_s.real / base.psi, mtpa.psi_s.imag / base.psi, label="MTPA")
        try:
            ax.plot(
                mtpv.psi_s.real / base.psi, mtpv.psi_s.imag / base.psi, label="MTPV"
            )
