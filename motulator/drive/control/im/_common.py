...
class ObserverCfg:
    ...
    def __post_init__(self, alpha_o, k_o):
        ...
        if self.sensorless:
            k_o = (
                (lambda w_m: (0.5 * alpha + 0.2 * np.abs(w_m)) / (alpha - 1j * w_m))
                if k_o is None
                else k_o
            )
        else:
            k_o = (
                (lambda w_m: 1 + 0.2 * np.abs(w_m) / (alpha - 1j * w_m))
                if k_o is None
                else k_o
            )
        # Collect the gains
...
class Observer:
    ...
    def output(self, fbk):
        ...
        self._work.d_psi_R = np.real(v + k_o1 * (v_r - v) + k_o2 * np.conj(v_r - v))
...
class FullOrderObserver:
    ...
    def output(self, fbk):
        ...
        p_term = (
            (-gain.alpha_o * par.L_sgm * err_i_s.imag / fbk.psi_R)
            if fbk.psi_R > 0
            else 0
        )
        ...
        self._work.d_i_s = (
            fbk.u_s
            - (par.R_sgm + 1j * fbk.w_s * par.L_sgm) * self.est.i_s
            + (par.alpha - 1j * w_m) * fbk.psi_R
            + k_i * err_i_s
        ) / par.L_sgm
        self._work.d_psi_R = (
            -par.alpha * fbk.psi_R
            + par.R_R * fbk.i_s.real
