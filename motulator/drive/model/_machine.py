...
class InductionMachine(Subsystem):
    ...
    def post_process_states(self):
        ...
        data = self.data
        L_s = (
            self.par.L_s(np.abs(data.psi_ss))
            if callable(self.par.L_s) else self.par.L_s)
...
class SynchronousMachine(Subsystem):
    ...
    def i_s(self):
        ...
        return (
            self.state.psi_s.real - self.par.psi_f
        )/self.par.L_d + 1j*self.state.psi_s.imag/self.par.L_q
    ...
    def post_process_states(self):
        ...
        if callable(self._i_s):
            ...
            data.i_s = (
                data.psi_s.real -
                par.psi_f)/par.L_d + 1j*data.psi_s.imag/par.L_q
